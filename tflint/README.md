...
the `TFLINT_VERSION` build arg. Renovate has no built-in manager for tflint version
files, so a `customManagers` regex entry in [renovate.json](../renovate.json) keeps
that file current instead; the published image tag tracks it directly. The binary
is downloaded directly from the tflint GitHub releases and verified against its
